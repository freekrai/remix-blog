...
declare global {
	...
	var lruCache:
	  | (LRUCache<string, {metadata: CacheMetadata; value: any}> & {name: string})
	  | undefined
}
...
function createLruCache() {
	...
	const newCache = new LRUCache<string, {metadata: CacheMetadata; value: any}>({
		max: 1000,
...
>(options: {
  ...
  checkValue?: (value: Value) => boolean | string | object
...
// v1 of caching... just here for documentation purposes really...
/*
declare global {
	var __cache: LRUCache<string, any>;
}

export const cache = global.__cache ||= new LRUCache({ max: 10000, ttl: 1000 * 60 * 10 });
